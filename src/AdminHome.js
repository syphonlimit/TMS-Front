...
//Props for react-select field for 'Create Group'
const components = {
...
export default function AdminHome() {
  ...
  //Handle the key down events for react-select field for 'Create Group'
  ...
  //Returns the created values from react-select field for 'Create Group'
    ...
    if (disabled) {
      ...
      try {
        const response = await axios.put(
          "http://localhost:8080/controller/updateUser/" + row.username,
          body,
          config
        );
  ...
  //Handling the toggling of enable/disable
  async function handleDisable(e, row) {
    e.preventDefault();
    const user = row.username;
    try {
      const response = await axios.put(
        "http://localhost:8080/controller/toggleUserStatus/" + user,
        {},
        config
      );
      toast.success(response.data.message);
  ...
  //Handling of register func, notifies the user after an action
  ...
  //Handling of createGroup func, notifies the user after an action
          ...
          <TableCell align="center">
            <Select
              ...
              value={groupOptions.filter((option) =>
                users.group_list.split(",").includes(option.value)
              )}
            />
          </TableCell>
          ...
          <TableRow key={item.username} noValidate>
            ...
            <TableCell align="center">
              <Select
                ...
                //read from users.group_list and match exactly
                value={groupOptions.filter((option) =>
                  item.group_list.split(",").includes(option.value)
                )}
              />
            </TableCell>
